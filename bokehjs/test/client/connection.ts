...
function promise_with_methods(): PromiseWithMethods<unknown> {
...
function with_server<T>(f: (x: {url: string}) => T): Promise<unknown> {
  const promise = promise_with_methods()
  ...
  const handle = child_process.spawn("python", ["-m", "bokeh", "serve", `--port=${port}`], {
    env: env,
    cwd: basedir,
  })
  handle.on('close', () => {
...
describe("ClientSession", function() {
  ...
  it("should pass request string to connection", () => {
    const promise = with_server((server_process) => {
      pull_session(server_process.url, undefined, "foo=10&bar=20").then(
        (session) => {
  ...
  it("should sync a document between two connections", () => {
    const promise = with_server((server_process) => {
      ...
      added_root.then(
        (session1) => {
          pull_session(server_process.url, session1.id).then(
            (session2) => {
              try {
